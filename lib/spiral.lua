...
function Spiral:init_params()
  params:add_group("spiral "..self.id, 14)
...
function Spiral:all_notes_off()
  if (self:get_param("output") == 2 or self:get_param("output") == 3) then
    for _, a in pairs(self.active_notes) do
      self.midi_out_device:note_off(a, nil, self:get_param("midi_out_channel"))
    end
  end
  self.active_notes = {}
end
...
function Spiral:step()
  while true do
    ...
    if self.playing then
      ...
      -- Audio engine out
      if self:get_param("output") == 1 or self:get_param("output") == 3 then
        engine.hz(freq)
        -- chord mode
      ...
      elseif self:get_param("output") == 5 then
      ...
      if (self:get_param("output") == 2 or self:get_param("output") == 3) then
        ...
        -- Note off timeout
        if self:get_param("note_length") < 4 then
